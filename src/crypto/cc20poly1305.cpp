...
void ComputeTag(ChaCha20&                 chacha20, 
               ...
               std::span<std::byte>       tag) noexcept 
{
   ...
   utils::WriteLE(length_desc, aad.size());
   utils::WriteLE(length_desc + 8, cipher.size());
...
void AEADChaCha20Poly1305::Encrypt(std::span<const std::byte> plain1, 
                                   ...
                                   std::span<std::byte>       plain2) noexcept 
{
   ...
   if (timingsafe_bcmp_internal(reinterpret_cast<uint8_t*>(expected_tag), 
                                reinterpret_cast<const uint8_t*>(cipher.last(EXPANSION).data()), 
                                EXPANSION))
   { return false; }
