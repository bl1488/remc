...
// common wrapper
class KeysSessionFixture : public ::testing::Test {
protected:
...
protected:
   static remc::crypto::SessionsKeys server_key, 
                                     client_key;
};

remc::crypto::SessionsKeys KeysSessionFixture::server_key;
remc::crypto::SessionsKeys KeysSessionFixture::client_key;

TEST_F(KeysSessionFixture, ChaCha20_BasicCheck) {
   remc::crypto::ChaCha20 cipher(
      (void*)server_key.GetSharedKey().data(), server_key.GetSharedKey().size());
...
TEST_F(KeysSessionFixture, FSCC20Poly1305_BasicCheck) {
   ...
   // client side
   remc::crypto::FSChaCha20Poly1305 poly_client(
      (void*)client_key.GetSharedKey().data(),  client_key.GetSharedKey().size());
